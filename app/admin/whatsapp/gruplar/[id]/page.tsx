        ...
        <div className="rounded-3xl border bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Excel seçilince sadece önizleme yapılır. Gruba eklemek için ayrıca onay vermen gerekir.
            </p>
    ...
    <td className="p-3">
      <button
        ...
        onClick={async () => {
          ...
          if (!contactId) {
            alert("Kişi bilgisi yok");
            return;
          }

          const ok = confirm(
            "Bu kişiyi bu gruptan çıkarmak istiyor musun?"
          );
          if (!ok) return;

          const res = await fetch(
            "/api/admin/whatsapp/group-members/remove",
            {
              method: "POST",
              headers: {
          ...
          if (data.success) {
            alert("Kişi gruptan çıkarıldı");
            loadMembers(); // SAYFAYI YENİLE
          } else {
